...
class UniverCrearTicketsViewController: UIViewController {
    ...
    @IBAction func crearTicket(_ sender: UIButton) {
        ...
        //Guardar el ticket
        let registrarTicket = self.ticketsTabla.insert(self.idUsuarioExp <- self.idUsuario!, self.fechaTicketExp <- self.fechaActual!, self.consultaExp <- self.consultaTextField.text!, estatusExp <- "Nuevo", calificacionExp <- 0, self.idUsuarioBibliotecarioExp <- 0)
    ...
    //https://bitbucket.org/ramses7083/rebuc-app/src
